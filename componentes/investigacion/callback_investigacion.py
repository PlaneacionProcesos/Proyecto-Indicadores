...
def registrar_callback_investigacion(app):
    ...
    # ==========================================================================
    # Callback de Alto Rendimiento (Tabla de Investigación)
    # ==========================================================================
    @app.callback(
    ...
    def datos_tabla_investigacion(
        año, centro, periodo, nivel, modalidad, tipo, tiempo_reporte,
        seccion_actual
    ):
        # 1. Guard de Sección
        if seccion_actual is not None and seccion_actual != "investigacion":
            return dash.no_update

        # 2. Filtrar DataFrame del agrupador
        datos_ae = resultados_completos[
            resultados_completos["agrupador"] == "Investigacion"
        ].copy()
    ...
    # ==========================================================================
    # Callback para Controlar Apertura y Cierre del Modal de Investigación
    # ==========================================================================
    @app.callback(
        Output("modal-contexto-investigacion", "style"),
        Output("modal-titulo-investigacion", "children"),
        Output("modal-cuerpo-investigacion", "children"),
        Output("tabla-investigacion", "active_cell"),
        Output("store-numero-ind-modal-investigacion", "data"),
        Output("alerta-doc-modal-investigacion", "children"),
        Input("tabla-investigacion", "active_cell"),
        Input("btn-cerrar-modal-investigacion", "n_clicks"),
        Input("btn-entendido-modal-investigacion", "n_clicks"),
        Input("seccion-actual", "data"),
        State("tabla-investigacion", "data"),
        prevent_initial_call=True,
    )
    def controlar_modal_investigacion(active_cell, n_close_x, n_close_btn, seccion_actual, data_tabla):
        if seccion_actual is not None and seccion_actual != "investigacion":
        ...
        if ctx_id in ("btn-cerrar-modal-investigacion", "btn-entendido-modal-investigacion"):
            return {"display": "none"}, "", "", None, None, ""

        # Si el disparador fue la tabla y hay una celda seleccionada
        if ctx_id == "tabla-investigacion" and active_cell is not None and data_tabla:
    ...
    # ==========================================================================
    @app.callback(
        Output("download-modal-investigacion", "data"),
        Output("alerta-doc-modal-investigacion", "children", allow_duplicate=True),
        Input("btn-descargar-sgc-investigacion", "n_clicks"),
        Input("btn-descargar-estrategico-investigacion", "n_clicks"),
        State("store-numero-ind-modal-investigacion", "data"),
        prevent_initial_call=True,
    )
    def descargar_documento_modal_investigacion(n_sgc, n_estrategico, numero_ind):
        ...
        tipo = "SGC" if trigger == "btn-descargar-sgc-investigacion" else "Estrategicos"
