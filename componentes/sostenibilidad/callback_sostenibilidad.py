...
def registrar_callback_sostenibilidad(app):
    ...
    # ==========================================================================
    # Callback de Alto Rendimiento (Tabla de Sostenibilidad)
    # ==========================================================================
    @app.callback(
        Output("tabla-sostenibilidad", "data"),
    ...
    def datos_tabla_sostenibilidad(
        año, centro, periodo, nivel, modalidad, tipo, tiempo_reporte,
        seccion_actual
    ):
        # 1. Guard de Sección
        if seccion_actual is not None and seccion_actual != "sostenibilidad":
            return dash.no_update

        # 2. Filtrar DataFrame del agrupador
        datos_ae = resultados_completos[
            resultados_completos["agrupador"] == "Sostenibilidad"
        ].copy()
    ...
    # ==========================================================================
    # Callback para Controlar Apertura y Cierre del Modal de Sostenibilidad
    # ==========================================================================
    @app.callback(
        Output("modal-contexto-sostenibilidad", "style"),
        Output("modal-titulo-sostenibilidad", "children"),
        Output("modal-cuerpo-sostenibilidad", "children"),
        Output("tabla-sostenibilidad", "active_cell"),
        Output("store-numero-ind-modal-sostenibilidad", "data"),
        Output("alerta-doc-modal-sostenibilidad", "children"),
        Input("tabla-sostenibilidad", "active_cell"),
        Input("btn-cerrar-modal-sostenibilidad", "n_clicks"),
        Input("btn-entendido-modal-sostenibilidad", "n_clicks"),
        Input("seccion-actual", "data"),
        State("tabla-sostenibilidad", "data"),
        prevent_initial_call=True,
    )
    def controlar_modal_sostenibilidad(active_cell, n_close_x, n_close_btn, seccion_actual, data_tabla):
        if seccion_actual is not None and seccion_actual != "sostenibilidad":
        ...
        if ctx_id in ("btn-cerrar-modal-sostenibilidad", "btn-entendido-modal-sostenibilidad"):
            return {"display": "none"}, "", "", None, None, ""

        # Si el disparador fue la tabla y hay una celda seleccionada
        if ctx_id == "tabla-sostenibilidad" and active_cell is not None and data_tabla:
    ...
    # ==========================================================================
    @app.callback(
        Output("download-modal-sostenibilidad", "data"),
        Output("alerta-doc-modal-sostenibilidad", "children", allow_duplicate=True),
        Input("btn-descargar-sgc-sostenibilidad", "n_clicks"),
        Input("btn-descargar-estrategico-sostenibilidad", "n_clicks"),
        State("store-numero-ind-modal-sostenibilidad", "data"),
        prevent_initial_call=True,
    )
    def descargar_documento_modal_sostenibilidad(n_sgc, n_estrategico, numero_ind):
        ...
        tipo = "SGC" if trigger == "btn-descargar-sgc-sostenibilidad" else "Estrategicos"
