...
def generar_tarjetas_contexto_impacto(datos_seccion, indicador_seleccionado=None):
    """
    ...
    for nombre in nombres_indicadores:
        ...
        df_meta = resultados_completos[
            (resultados_completos["agrupador"] == "Impacto") &
...
def registrar_callback_impacto(app):
    ...
    # ==========================================================================
    # Callback Unificado de Alto Rendimiento (Tabla + Tarjetas de Impacto)
    # ==========================================================================
    @app.callback(
        Output("tabla-impacto", "data"),
        Output("tarjetas-contexto-impacto", "children"),
        ...
        Input("tabla-impacto", "active_cell"),
        Input("seccion-actual", "data"),
        dash.State("tabla-impacto", "data"),
    )
    def datos_y_tarjetas_impacto(
        año, centro, periodo, nivel, modalidad, tipo, tiempo_reporte,
        active_cell, seccion_actual, data_tabla_actual
    ):
        # 1. Guard de Sección
        if seccion_actual is not None and seccion_actual != "impacto":
        ...
        if disparador == "tabla-impacto" and data_tabla_actual:
            indicador_sel = None
            ...
            return dash.no_update, generar_tarjetas_contexto_impacto(datos_ae, indicador_seleccionado=indicador_sel)
        ...
        if datos_ae.empty:
            ...
            return df_resultado.to_dict("records"), generar_tarjetas_contexto_impacto(datos_ae)
        ...
        tarjetas_ui = generar_tarjetas_contexto_impacto(datos_ae, indicador_seleccionado=indicador_seleccionado)

        return records, tarjetas_ui

    # ==========================================================================
    # Callback para Controlar Apertura y Cierre del Modal de Impacto
    # ==========================================================================
    @app.callback(
        Output("modal-contexto-impacto", "style"),
        Output("modal-titulo-impacto", "children"),
        Output("modal-cuerpo-impacto", "children"),
        Input("tabla-impacto", "active_cell"),
        Input("btn-cerrar-modal-impacto", "n_clicks"),
        Input("btn-entendido-modal-impacto", "n_clicks"),
        Input("seccion-actual", "data"),
        dash.State("tabla-impacto", "data"),
        prevent_initial_call=True,
    )
    def controlar_modal_impacto(active_cell, n_close_x, n_close_btn, seccion_actual, data_tabla):
        if seccion_actual is not None and seccion_actual != "impacto":
            return {"display": "none"}, "", ""

        ctx_id = dash.ctx.triggered_id
        if ctx_id in ("btn-cerrar-modal-impacto", "btn-entendido-modal-impacto"):
            return {"display": "none"}, "", ""

        if ctx_id == "tabla-impacto" and active_cell is not None and data_tabla:
            row_idx = active_cell.get("row")
